$(document).ready(function() {
	...
	reqRankChina(resRankChina);
});
var table = null;

function resRankChina(data) {
	console.log(data)
...
$('.btn-item').on('click', function() {
	...
	switch(flag) {
		case '0':
			...
			reqRankChinaHandle(resRankChinaHandle, flag, sid, '', '', '', '', '');
			break;
		case '1':
			var sid = table.row('.selected').data().sid;
			$('.mask').show();
			$('.btn-yes').on('click', function() {
				...
				var rank = $('#rank').val();
				reqRankChinaHandle(resRankChinaHandle, flag, sid, songName, songFrom, songUrl, author, rank);
			});
			break;
		case '2':
			$('.mask').show();
...
function resRankChinaHandle(data) {
	console.log(data);
